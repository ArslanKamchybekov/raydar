...
const LocationPage = () => {
      ...
      map.addLayer({
        ...
        paint: {
          "fill-extrusion-color":
            currentTheme === "dark"
              ? ["interpolate", ["linear"], ["get", "height"], 0, "#242424", 500, "#454545"]
              : ["interpolate", ["linear"], ["get", "height"], 0, "#aaa", 500, "#858585"],
  ...
  useEffect(() => {
    if (!mapLoaded || !mapRef.current || !items) return

    markersRef.current.forEach((marker) => marker.remove())
    markersRef.current = []

    const locationCounts = locations.map((location) => {
      const itemsAtLocation = items.filter((item) => item.location === location.name)

      const weatherCounts = itemsAtLocation.reduce<Record<string, number>>((acc, item) => {
        const weather = item.weather || "unknown"
        acc[weather] = (acc[weather] || 0) + 1
        return acc
      }, {})

      const weatherMostLost = Object.entries(weatherCounts).reduce<string>((max, [weather, currentCount]) => {
        const maxCount = weatherCounts[max] || 0
        return currentCount > maxCount ? weather : max
      }, Object.keys(weatherCounts)[0] || "No data")

      const categoryCounts = itemsAtLocation.reduce<Record<string, number>>((acc, item) => {
        const category = item.category || "unknown"
        acc[category] = (acc[category] || 0) + 1
        return acc
      }, {})

      const categoryMostLost = Object.entries(categoryCounts).reduce<string>((max, [category, currentCount]) => {
      ...
      const popup = new mapboxgl.Popup({
        ...
        <div class="p-4 ${currentTheme === "dark" ? "text-white bg-gray-800" : "text-gray-800 bg-white"}">
  ...
  return (
    <div className="relative h-screen">
      ...
      <p className="leading-7 text-sm text-muted-foreground mb-4">Find out where most items are lost in your area.</p>

      <div ref={mapContainerRef} className="h-[80vh] w-full rounded-lg overflow-hidden shadow-lg" />

      {topLocation && (
        <div className="absolute bottom-0 left-0 pb-20 mb-16 w-full sm:w-auto sm:left-4 md:left-6 lg:left-8">
          <Card className="shadow-lg backdrop-blur-sm bg-opacity-90 w-full sm:w-[280px] md:w-[320px] lg:w-[360px]">
            <CardHeader className="p-3 sm:p-4">
              <CardTitle className="text-base sm:text-lg">Where Are Most Items Lost?</CardTitle>
            </CardHeader>
            <CardContent className="p-3 sm:p-4 space-y-1.5">
              ...
              <p className="text-xs sm:text-sm flex justify-between items-center">
                ...
                <span>{topLocation.weatherMostLost}</span>
              </p>
              <p className="text-xs sm:text-sm flex justify-between items-center">
                <span className="font-semibold">Category:</span>
                <span>{topLocation.categoryMostLost}</span>
              </p>
            </CardContent>
          </Card>
        </div>
